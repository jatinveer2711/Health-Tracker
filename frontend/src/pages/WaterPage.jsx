...
//get enteries
  const fetchEntries = async()=>{
    try {
      const res = await axios.get('https://health-tracker-1-5pod.onrender.com/api/water/getAll',{
        headers:{
...
const handleSumbit = async(e)=>{
  e.preventDefault();
  try {
     await axios.post('https://health-tracker-1-5pod.onrender.com/api/water/create',{amount},{
      headers:{
        Authorization:`Bearer ${token}`
      }
    })
...
const handleDelete = async(id)=>{
  try {
    const res = await axios.delete(`https://health-tracker-1-5pod.onrender.com/api/water/delete/${id}`,{
      headers:{
        Authorization:`Bearer ${token}`
      }
...
const hanldeUpdate = async(e)=>{
  ...
  try {
    if(!editId) return;
    await axios.put(`https://health-tracker-1-5pod.onrender.com/api/water/update/${editId}`,editamount,{
      headers:{
        Authorization:`Bearer ${token}`
      }
    })

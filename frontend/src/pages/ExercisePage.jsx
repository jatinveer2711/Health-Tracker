...
export default function Exercise() {
  ...
  const fetchExercises = async () => {
    ...
    setError(null);
    try {
      const res = await axios.get("https://health-tracker-1-5pod.onrender.com/api/exercise/getAll", {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post("https://health-tracker-1-5pod.onrender.com/api/exercise/create", exercise, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleDelete = async(id)=>{
    try {
      const res = await axios.delete(`https://health-tracker-1-5pod.onrender.com/api/exercise/delete/${id}`,{
        headers:{
  ...
  const hanldeUpdate = async(e)=>{
    ...
    try {
      if(!editId) return;
      await axios.put(`https://health-tracker-1-5pod.onrender.com/api/exercise/update/${editId}`,editExercise,{
        headers:{
          Authorization: `Bearer ${token}`
        }
      }

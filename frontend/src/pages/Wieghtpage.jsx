...
export default function WeightPage() {
  ...
  const fetchWeights = async () => {
    try {
      const res = await axios.get("https://health-tracker-1-5pod.onrender.com/api/weight/getAll", {
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(
        "https://health-tracker-1-5pod.onrender.com/api/weight/create",
  ...
  const handleDelete = async(id)=>{
    try {
      const res = await axios.delete(`https://health-tracker-1-5pod.onrender.com/api/weight/delete/${id}`,{
        headers:{
          Authorization:`Bearer ${token}`
        }
      })
  ...
  const handleUpdate = async(e)=>{
    ...
    try {
      if(!editId) return;
      await axios.put(`https://health-tracker-1-5pod.onrender.com/api/weight/update/${editId}`,editWeight,{
    headers:{
      Authorization:`Bearer ${token}`
    }})
    setEditID(null)

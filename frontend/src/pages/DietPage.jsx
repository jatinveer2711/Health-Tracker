...
export default function DietPage() {
  ...
  const fetchDiet = async () => {
    try {
      const res = await axios.get('https://health-tracker-1-5pod.onrender.com/api/diet/getAll', {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post('https://health-tracker-1-5pod.onrender.com/api/diet/create', foodata, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleDelete = async(id)=>{
    try {
      const res = await axios.delete(`https://health-tracker-1-5pod.onrender.com/api/diet/delete/${id}`,{
        headers:{
  ...
  const hanldeUpdate = async(e)=>{
    ...
    try {
      if(!editId) return;
      const res = await axios.put(`https://health-tracker-1-5pod.onrender.com/api/diet/update/${editId}`,editDiet,{
        headers:{
          Authorization:`Bearer ${token}`
        }
      })
